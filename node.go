...
func main() {
	...
	if *swarmKeyPath != "" {
		...
		if err != nil {
			log.Fatalf("pnet: load swarm.key: %v", err)
		}
		...
		go func() {
			...
			for {
				...
				case <-t.C:
					...
					if err != nil {
						log.Printf("dev: build: %v", err)
						continue
					}
					if err := txTopic.Publish(ctx, coseBytes); err != nil {
...
func validateCOSETx(b []byte) (string, string, int64, error) {
	...
	case map[int]interface{}:
		...
		if err != nil {
			return "", "", 0, fmt.Errorf("cose: protected marshal: %w", err)
		}
		prot = b
	...
	kidv, ok := ph[4]
	if !ok {
		return "", "", 0, fmt.Errorf("cose: missing kid")
	}
	kid, ok := kidv.([]byte)
	if !ok {
		return "", "", 0, fmt.Errorf("cose: kid type")
	}
	...
	if err != nil {
		return "", "", 0, fmt.Errorf("cose: sig-struct: %w", err)
	}

	// Verify signature
	pub, ok := registryGet(kid)
	if !ok {
		return "", "", 0, fmt.Errorf("registry: unknown kid %s", hex.EncodeToString(kid))
	}
	...
	// types
	if _, ok := plRaw[0].(int64); !ok && !isUint(plRaw[0]) {
		return "", "", 0, fmt.Errorf("payload[0] version int")
	}
	if _, ok := plRaw[1].(string); !ok {
		return "", "", 0, fmt.Errorf("payload[1] network_id string")
	}
	if _, ok := plRaw[2].(string); !ok {
		return "", "", 0, fmt.Errorf("payload[2] tx_type string")
	}
	devMap, ok := asIntKeyedMap(plRaw[3])
	if !ok {
		return "", "", 0, fmt.Errorf("payload[3] device map")
	}
	seq := toInt64(plRaw[4])
	if seq <= 0 {
		return "", "", 0, fmt.Errorf("payload[4] seq > 0")
	}
	// fee
	feeMap, ok := asIntKeyedMap(plRaw[6])
	if !ok {
		return "", "", 0, fmt.Errorf("payload[6] fee map")
	}
	amt := toInt64(feeMap[int64(0)])
	denom, _ := feeMap[int64(1)].(string)
	if amt < 1 || denom != "uCR" {
		return "", "", 0, fmt.Errorf("fee invalid")
	}
	if _, ok := asIntKeyedMap(plRaw[8]); !ok {
		return "", "", 0, fmt.Errorf("payload[8] map")
	}

	// device id
	devID, _ := devMap[int64(0)].(string)
	if devID == "" {
		return "", "", 0, fmt.Errorf("device id missing")
	}
