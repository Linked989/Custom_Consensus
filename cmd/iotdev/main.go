    ...
    for i := range devs {
        ...
        go func(d *device) {
            ...
            for {
                ...
                if err != nil { log.Printf("build error (%s): %v", d.id, err); return }
                req, _ := http.NewRequest(http.MethodPost, *base+"/tx", bytes.NewReader(cose))
                req.Header.Set("Content-Type", "application/cbor")
                resp, err := cli.Do(req)
                if err != nil { log.Printf("post error (%s): %v", d.id, err); return }
...
func short(s string) string { if len(s) > 12 { return s[len(s)-12:] }; return s }
