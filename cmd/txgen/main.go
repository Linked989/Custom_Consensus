...
func main() {
    ...
    if *register {
        ...
        if err := doPostJSON(regURL, []byte(body)); err != nil { log.Fatalf("register: %v", err) }
    ...
    for i := 0; i < *count; i++ {
        ...
        if err != nil { log.Fatalf("post: %v", err) }
...
func replacePath(url, path string) string {
    ...
    if len(url) >= 3 && url[len(url)-3:] == "/tx" { return url[:len(url)-3] + path }
    return url + path
}
