...
func UpdateLastSeq(devID string, kid []byte, seq int64) bool {
	devSeq.mu.Lock()
	key := seqKey(devID, kid)
	last := devSeq.m[key]
	if seq > last {
		devSeq.m[key] = seq
		devSeq.mu.Unlock()
...
func seqKey(devID string, kid []byte) string {
	if len(kid) == 0 {
		return devID
	}
	return devID + "|" + hex.EncodeToString(kid)
}
...
// ValidateCOSETx verifies COSE_Sign1(Ed25519) and returns txid/devID/kid/seq.
func ValidateCOSETx(b []byte) (string, string, []byte, int64, error) {
	var tag cbor.Tag
	var arr []interface{}
	if err := decMode.Unmarshal(b, &tag); err == nil && tag.Number == 18 {
		var ok bool
		if arr, ok = tag.Content.([]interface{}); !ok {
			return "", "", nil, 0, fmt.Errorf("cose: bad content")
		}
	} else {
		if err := decMode.Unmarshal(b, &arr); err != nil {
			return "", "", nil, 0, fmt.Errorf("cose: decode: %w", err)
		}
	}
	if len(arr) != 4 {
		return "", "", nil, 0, fmt.Errorf("cose: array len %d", len(arr))
	}
	...
	if err := decMode.Unmarshal(prot, &ph); err != nil {
		return "", "", nil, 0, fmt.Errorf("cose: protected map: %w", err)
	}
	if alg, ok := ph[1]; !ok || toInt64(alg) != -8 {
		return "", "", nil, 0, fmt.Errorf("cose: alg != -8")
	}
	kidv, ok := ph[4]
	if !ok {
		return "", "", nil, 0, fmt.Errorf("cose: missing kid")
	}
	kid, ok := kidv.([]byte)
	if !ok {
		return "", "", nil, 0, fmt.Errorf("cose: kid type")
	}

	// Sig_structure
	toSign, err := encMode.Marshal([]interface{}{"Signature1", prot, []byte{}, payload})
	if err != nil {
		return "", "", nil, 0, fmt.Errorf("cose: sig-struct: %w", err)
	}

	pub, ok := RegistryGet(kid)
	if !ok {
		return "", "", nil, 0, fmt.Errorf("registry: unknown kid")
	}
	...
	if _, ok := pl[0].(int64); !ok && !isUint(pl[0]) {
		return "", "", nil, 0, fmt.Errorf("payload[0] version int")
	}
	if _, ok := pl[1].(string); !ok {
		return "", "", nil, 0, fmt.Errorf("payload[1] network_id string")
	}
	if _, ok := pl[2].(string); !ok {
		return "", "", nil, 0, fmt.Errorf("payload[2] tx_type string")
	}
	devMap, ok := asIntKeyedMap(pl[3])
	if !ok {
		return "", "", nil, 0, fmt.Errorf("payload[3] device map")
	}
	seq := toInt64(pl[4])
	if seq <= 0 {
		return "", "", nil, 0, fmt.Errorf("payload[4] seq > 0")
	}
	feeMap, ok := asIntKeyedMap(pl[6])
	if !ok {
		return "", "", nil, 0, fmt.Errorf("payload[6] fee map")
	}
	amt := toInt64(feeMap[0])
	denom, _ := feeMap[1].(string)
	if amt < 1 || denom != "uCR" {
		return "", "", nil, 0, fmt.Errorf("fee invalid")
	}
	if _, ok := asIntKeyedMap(pl[8]); !ok {
		return "", "", nil, 0, fmt.Errorf("payload[8] map")
	}

	devID, _ := devMap[0].(string)
	if devID == "" {
		return "", "", nil, 0, fmt.Errorf("device id missing")
	}
	txid := sha256.Sum256(payload)
	kidCopy := append([]byte(nil), kid...)
	return hex.EncodeToString(txid[:]), devID, kidCopy, seq, nil
}
...
func ExtractKid(b []byte) ([]byte, error) {
	...
	if err := decMode.Unmarshal(b, &tag); err == nil && tag.Number == 18 {
		var ok bool
		if arr, ok = tag.Content.([]interface{}); !ok {
			return nil, fmt.Errorf("cose: bad content")
		}
	} else {
		if err := decMode.Unmarshal(b, &arr); err != nil {
			return nil, err
	...
	if len(arr) != 4 {
		return nil, fmt.Errorf("cose: array len %d", len(arr))
	}
	var prot []byte
	switch v := arr[0].(type) {
	case []byte:
	...
	kidv, ok := ph[4]
	if !ok {
		return nil, fmt.Errorf("cose: missing kid")
	}
	kid, ok := kidv.([]byte)
	if !ok {
		return nil, fmt.Errorf("cose: kid type")
	}
	return kid, nil
}
