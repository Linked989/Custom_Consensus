...
// MaxDevices returns the configured device ceiling (0 = unlimited).
...
// SetMaxDevices updates the device ceiling.
func (m *Manager) SetMaxDevices(v int) {
	m.mu.Lock()
	m.maxDevices = v
	m.mu.Unlock()
}

// CanAccept reports whether the manager allows another device given count.
func (m *Manager) CanAccept(count int) bool {
	m.mu.Lock()
	max := m.maxDevices
	m.mu.Unlock()
...
func (m *Manager) Status() *Cell {
	...
	defer m.mu.Unlock()
	if m.cell == nil {
		return &Cell{ID: "", NodeID: m.nodeID, ChainID: m.chainID, Devices: nil, Threshold: m.threshold, MaxDevices: m.maxDevices, Active: false}
	}
	cpy := *m.cell
	return &cpy
}
...
func (m *Manager) TryForm(reg *iot.Registry) *Cell {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.cell != nil && m.cell.Active {
		return nil
	}
	devs := reg.List()
	if len(devs) < m.threshold {
		return nil
	}
	...
	n := m.threshold
	if m.maxDevices > 0 && m.maxDevices < n {
		n = m.maxDevices
	}
