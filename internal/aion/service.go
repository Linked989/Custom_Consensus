...
func (s *LeaderService) loop(ctx context.Context) {
    if s.interval <= 0 { s.interval = 2 * time.Second }
...
func (s *LeaderService) evalOnce() {
    ...
    var pubBytes []byte
    if pub != nil { if b, err := crypto.MarshalPublicKey(pub); err == nil { pubBytes = b } }
    hh := sha256.New()
    hh.Write(ch[:])
    hh.Write(pubBytes)
    var y [32]byte
    ...
    s.mu.Lock(); s.lastThr16 = thr; s.lastRank16 = val; s.lastEpoch = epoch; s.mu.Unlock()
    elected := val < thr

    if elected {
        SetLeaderActive(true)
        s.mu.Lock(); s.leaderActive = true; s.leaderEpoch = int64(epoch); s.mu.Unlock()
