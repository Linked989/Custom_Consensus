...
func ComputeRoot(leaves [][]byte) []byte {
    ...
    for i := range leaves { b := make([]byte, len(leaves[i])); copy(b, leaves[i]); level[i] = b }
    for len(level) > 1 {
...
func VerifyProof(leaf []byte, proof [][]byte, index int, expectedRoot []byte) bool {
    ...
    if len(hash) != len(expectedRoot) { return false }
    for i := range hash { if hash[i] != expectedRoot[i] { return false } }
    return true
}
